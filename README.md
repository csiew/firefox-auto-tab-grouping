...
- **Grouped Tab Regrouping Control**: Choose whether already-grouped tabs can move into matching rule groups (default: regroup grouped tabs enabled)
...
3. **Configure Settings** in the settings card:
   ...
   - **Regroup grouped tabs toggle**: Choose whether tabs already in a group can move into matching rule groups (default: enabled)
...
6. **Automatic Grouping**: When you visit a configured pattern:
   ...
   - Tabs already in a group can stay in that group when Regroup grouped tabs is disabled
...
4. **Manage Groups**: Creates groups with custom names and colors, ungroups tabs outside configured rules when Strict rules is enabled, and optionally preserves already-grouped tabs
